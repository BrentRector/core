...
async def test_entity_state(
    ...
    tasks_response.tasks.task_list = [
        ...
        _make_task_list(
            LIST_ID, 10, 2, "Ignored", deleted=datetime(2024, 1, 1, tzinfo=UTC)
        ),
    ]
    # Consolidate into one list response (both taskseries under list_id=LIST_ID)
    task_list = MagicMock()
    task_list.id = LIST_ID
    ts1 = MagicMock()
    ts1.id = 10
    ...
    t2 = MagicMock()
    t2.id = 2
    t2.completed = datetime(2024, 1, 1, tzinfo=UTC)
    t2.deleted = None
    t2.due = None
    t2.has_due_time = False
    ts2.task = [t2]
    ts3 = MagicMock()
    ts3.id = 30
    ts3.name = "Bread"
    ...
    t3.id = 3
    t3.completed = None
    t3.deleted = datetime(2024, 1, 1, tzinfo=UTC)
    t3.due = None
    t3.has_due_time = False
    ts3.task = [t3]
    task_list.taskseries = [ts1, ts2, ts3]
    tasks_response = MagicMock()
...
async def test_update_todo_item_rename(
    ...
    t.deleted = None
    t.due = None
    t.has_due_time = False
    ts.task = [t]
    task_list.taskseries = [ts]
    tasks_response = MagicMock()
    tasks_response.tasks.task_list = [task_list]
    client.rtm.tasks.get_list = AsyncMock(return_value=tasks_response)
...
@pytest.mark.usefixtures("storage")
async def test_setup_skips_non_list_subentries(
    hass: HomeAssistant,
    client: MagicMock,
) -> None:
