...
class RTMConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def _async_create_entry(
        ...
        auth = self._get_auth(api_key, shared_secret, token_data["token"])
        client = AioRTMClient(auth)
        try:
            lists_response = await client.rtm.lists.get_list()
        except AuthError, ResponseError:
            return self.async_abort(reason="cannot_connect")
        subentries: list[ConfigSubentryData] = [
            ConfigSubentryData(
                data={CONF_LIST_ID: lst.id},
                subentry_type=SUBENTRY_TYPE_LIST,
                title=lst.name,
                unique_id=str(lst.id),
            )
